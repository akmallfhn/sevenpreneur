...
function getVideoEmbed(url: string): { type: "iframe" | "video"; src: string } | null {
  ...
  try {
    ...
    if (loomMatch) return { type: "iframe", src: `https://www.loom.com/embed/${loomMatch[1]}` };
...
export default function SessionDetailAilene({ sessionId }: SessionDetailAileneProps) {
  ...
  const isExpired = session ? dayjs().isAfter(dayjs(session.meeting_date).add(4, "hour")) : false;

  const conferencePlatform = useMemo(
    () => (session?.meeting_url ? getConferenceVariantFromURL(session.meeting_url) : "UNKNOWN"),
    [session?.meeting_url]
  );
  const { conferenceIcon, conferenceName } = getConferenceAttributes(conferencePlatform);
  ...
  const locationDisplay = useMemo(() => {
    ...
    if (session.method === LearningMethodEnum.ONSITE) return session.location_name ?? "";
    return "Hybrid Meeting";
  }, [session, conferenceName]);

  return (
    <div className={`root hidden w-full min-h-screen overflow-y-auto lg:flex lg:flex-col pb-10 ${isCollapsed ? "pl-16" : "pl-64"}`}>
        ...
        <>
          ...
          <div className="relative w-full aspect-[5208/702] overflow-hidden">
            ...
            <div className="absolute inset-y-0 left-8 flex items-center gap-4 z-10">
              {session.method !== LearningMethodEnum.ONSITE && session.meeting_url && (
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <p className="font-bodycopy font-medium text-[15px] text-white/90 drop-shadow">
                  {dayjs(session.meeting_date).format("ddd, DD MMM YYYY - HH:mm")}
            ...
            <div className="absolute inset-y-0 right-8 flex items-center z-10">
              {session.method !== LearningMethodEnum.ONSITE && session.meeting_url && isUpcoming && (
                <a href={session.meeting_url} target="_blank" rel="noopener noreferrer">
                  <AppButton variant="light" size="medium" disabled={isExpired}>
                    <Video className="size-5" />
                    Launch Meeting
                  </AppButton>
                </a>
              )}
              {session.method === LearningMethodEnum.ONSITE && session.location_url && (
                ...
                {videoEmbed ? (
                  <div className="w-full aspect-video rounded-lg overflow-hidden bg-black">
                    {videoEmbed.type === "iframe" ? (
                      ...
                      <video src={videoEmbed.src} controls className="w-full h-full object-contain" />
                    )}
                  </div>
                ) : (
                  <div className="flex items-center justify-center py-10 rounded-lg border border-dashed border-dashboard-border">
                    <p className="font-bodycopy text-sm text-emphasis">
                      {isUpcoming ? "Recording akan tersedia setelah session selesai." : "Recording belum tersedia."}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3 bg-card-bg border border-dashboard-border p-5 rounded-lg">
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-bodycopy text-xs text-emphasis">Metode</span>
                    <span className={`text-[11px] font-semibold font-bodycopy px-2.5 py-0.5 rounded-full ${METHOD_BADGE[session.method].cls}`}>
                      {METHOD_BADGE[session.method].label}
                    </span>
                  </div>
                  {/* Date */}
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-bodycopy text-xs text-emphasis shrink-0">Tanggal</span>
                    <span className="font-bodycopy text-xs font-medium text-sevenpreneur-coal dark:text-white text-right">
                      {dayjs(session.meeting_date).format("dddd, D MMM YYYY · HH:mm")}
                    </span>
                  </div>
                  {/* Location */}
                  {session.location_name && (
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-bodycopy text-xs text-emphasis shrink-0">Lokasi</span>
                  ...
                  {/* Status */}
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-bodycopy text-xs text-emphasis shrink-0">Status</span>
                    <span className={`text-[11px] font-semibold font-bodycopy px-2.5 py-0.5 rounded-full ${isUpcoming ? "bg-tertiary/10 text-tertiary" : "bg-sevenpreneur-ash dark:bg-sevenpreneur-smoke text-emphasis"}`}>
                ...
                <a href={session.meeting_url} target="_blank" rel="noopener noreferrer">
                  <div className="flex items-center justify-between bg-card-bg border border-dashboard-border hover:border-primary/40 p-4 rounded-lg transition-colors cursor-pointer">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <p className="font-bodycopy text-xs text-emphasis">{conferenceName}</p>
                    </div>
                ...
                <a href={session.recording_url} target="_blank" rel="noopener noreferrer">
                  <div className="flex items-center justify-between bg-card-bg border border-dashboard-border hover:border-primary/40 p-4 rounded-lg transition-colors cursor-pointer">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <p className="font-bodycopy text-xs text-emphasis">Buka di tab baru</p>
                    </div>
                    <Video className="size-5 text-primary" />
                  </div>
                </a>
                  ...
                  <div className="flex items-center justify-between bg-card-bg border border-dashboard-border hover:border-primary/40 p-4 rounded-lg transition-colors cursor-pointer">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <div className="flex items-center gap-0.5 mt-0.5">
                        {[1,2,3,4,5].map((s) => (
                          <svg key={s} className="size-4" viewBox="0 0 20 20" fill="#FFB21D">
