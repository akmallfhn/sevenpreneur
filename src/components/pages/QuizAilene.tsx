...
function TimerRing({ timeLeft }: { timeLeft: number }) {
  ...
  const color =
    timeLeft > 60 ? "#534AB7" : timeLeft > 30 ? "#F59E0B" : "#EF4444";
  return (
    <div className="relative size-14 shrink-0">
      ...
      <span
        className="absolute inset-0 flex items-center justify-center font-bodycopy font-semibold text-[11px]"
        style={{ color }}
      >
        {formatTime(timeLeft)}
      </span>
...
export default function QuizAilene({ lessonId }: QuizAileneProps) {
  ...
  // Derive phase — auto-start when data is ready and user is a member
  ...
  const shouldAutoStart =
    !isLoading && !isError && isMember && !existingProgress?.completed_at;
  const effectivePhase: Phase = isAlreadyCompleted
    ? "result"
    : shouldAutoStart && phase === "idle"
      ? "active"
      : phase;
  ...
  const handleNext = () => {
    ...
    const correct = questions.filter((q) => {
      ...
      return (
        sel != null &&
        q.options.some((o) => o.is_correct && o.option_id === sel)
      );
    }).length;
    const score =
    ...
    submitMutation.mutate(
      { lesson_id: lessonId, score, answers },
      {
        onSuccess: (d) => {
          ...
          utils.ailene.listQuizQuestionsForUser.invalidate({
            lesson_id: lessonId,
          });
        },
      }
  ...
  return (
    <PageContainerCMS className="overflow-y-auto">
      <div className="w-full max-w-[880px] mx-auto flex flex-col gap-6">
        ...
        {/* ── IDLE — only shown when not a member ─────────── */}
        {!isLoading && !isError && effectivePhase === "idle" && (
          <p className="font-bodycopy text-sm text-danger py-20 text-center">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                {currentQuestion.options.map((opt) => {
                  ...
                  if (!isLocked) {
                    btnCls =
                      "border-sevenpreneur-ash hover:border-primary/50 hover:bg-section-background text-sevenpreneur-coal cursor-pointer";
                    badgeCls = "bg-sevenpreneur-ash text-sevenpreneur-coal";
                  } else if (isCorrect) {
                    btnCls =
                      "border-success bg-success-background/40 text-sevenpreneur-coal";
                    badgeCls = "bg-success text-white";
                  } else if (isWrongSelection) {
                    btnCls =
                      "border-danger bg-danger-background/40 text-sevenpreneur-coal";
                    badgeCls = "bg-danger text-white";
                  } else {
                    btnCls =
                      "border-sevenpreneur-ash/40 text-emphasis opacity-50";
              ...
              {isLocked && (
                  ...
                  className={`flex items-center gap-2 px-4 py-3 rounded-lg border font-bodycopy text-sm font-semibold animate-in fade-in slide-in-from-bottom-2 duration-200 ${
                    currentAnswer === null
                      ...
                      : currentQuestion.options.some(
                            (o) => o.is_correct && o.option_id === currentAnswer
                          )
                ...
                >
                  {currentAnswer === null ? (
                    <>
                      <Clock className="size-4 shrink-0" /> Waktu habis! Jawaban
                      yang benar disorot.
                    </>
                  ) : currentQuestion.options.some(
                      (o) => o.is_correct && o.option_id === currentAnswer
        ...
        {effectivePhase === "result" && (
          <div className="flex flex-col gap-6">
            ...
            >
              ...
              <p className="font-bodycopy text-[15px] text-emphasis">
            ...
            <div className="flex flex-col gap-3">
              ...
              {questions.map((q, idx) => {
                ...
                const isCorrect =
                  !isTimeout &&
                  q.options.some(
                    (o) => o.is_correct && o.option_id === selected
                  );
                const correctOpt = q.options.find((o) => o.is_correct);
                const selectedOpt = q.options.find(
                  (o) => o.option_id === selected
                );
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <p className="font-bodycopy font-semibold text-sm text-sevenpreneur-coal">
                        <span className="text-emphasis font-normal mr-1">
                          {idx + 1}.
                        </span>
                        {q.question}
                      </p>
                    ...
                    <div className="flex flex-col gap-1 pl-6">
                      {isTimeout ? (
                        <p className="font-bodycopy text-xs text-amber-700">
                          ...
                          <strong>
                            {correctOpt?.option_id.toUpperCase()}.{" "}
                            {correctOpt?.text}
                          </strong>
                        </p>
                      ) : isCorrect ? (
                        <p className="font-bodycopy text-xs text-success-foreground">
                          Jawaban kamu benar:{" "}
                          <strong>
                            {selectedOpt?.option_id.toUpperCase()}.{" "}
                            {selectedOpt?.text}
                          </strong>
                        </p>
                      ) : (
