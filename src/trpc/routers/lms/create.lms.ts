  ...
  cohort: roleBasedProcedure(["Administrator", "Class Manager"])
    .input(
      z.object({
        ...
        image_banner: stringNotBlank().optional(),
        image_square: stringNotBlank().optional(),
    ...
    .mutation(async (opts) => {
      ...
      const createdCohort = await opts.ctx.prisma.cohort.create({
        data: {
          ...
          image_banner: opts.input.image_banner || opts.input.image,
          image_square: opts.input.image_square || opts.input.image,
