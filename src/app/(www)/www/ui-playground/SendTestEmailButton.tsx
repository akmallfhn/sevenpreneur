...
// import { SendTestSessionReminder } from "@/lib/actions";
...
export default function SendTestEmailButton() {
  ...
  //   setErrorMsg(null);
  //   try {
  //     await SendTestSessionReminder(to);
  //     setStatus("success");
  //   } catch (err) {
  //     setStatus("error");
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-6 border border-dashed border-gray-300 rounded-xl max-w-sm mx-auto">
      ...
      >
        {status === "pending" ? "Sending..." : "Send Session Reminder"}
      </AppButton> */}
      {status === "success" && (
        <p className="text-xs text-green-600 font-medium">Email sent!</p>
      )}
      {status === "error" && (
