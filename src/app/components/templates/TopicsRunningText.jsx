...
export default function TopicsRunningText() {
    const topicsData = [
        ...
        {
            "topic_name": "Find the right investors for next business"
        },
    ]

    // Gandakan data agar bisa seamless scroll
    const loopedTopics = Array(6).fill(topicsData).flat()

    return(
        <div className="topics-running-text pb-8 flex flex-col gap-3 items-center">
                ...
                <div className="scroll-left flex gap-3 w-max">
                    {loopedTopics.map((post,index) => (
                        <div className="border-topic-item p-[1px] rounded-md bg-gradient-to-r from-0% from-[#727272] via-50% via-[#333333] to-100% to-[#727272]" key={index}>
                            <div className="topic-item flex items-start text-white font-bodycopy font-medium max-w-[200px] bg-[#1B1B1B] p-2 gap-2 rounded-md">
